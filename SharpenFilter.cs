...
internal class SharpenFilter : AbstractConvolutionalFilter
{
    private readonly float _sigma;

    public SharpenFilter(Bitmap originalImage, int kernelSize = 3, float sigma = 10.0f) : base(originalImage, kernelSize)
    {
        _sigma = sigma;
    }

    public unsafe Bitmap ApplyKernel()
    {
        Bitmap image = new Bitmap(_originalImage);
        float[,] LoGMatrix = GetKernelMatrix();
